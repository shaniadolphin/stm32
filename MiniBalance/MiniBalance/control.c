...
{    
	...
	{
		Get_Angle(Way_Angle);  					//10ms控制一次，为了保证M法测速的时间基准，首先读取编码器数据
		Encoder_Left = -Read_Encoder(2);       	//===读取编码器的值，因为两个电机的旋转了180度的，所以对其中一个取反，保证输出极性一致
		Encoder_Right =Read_Encoder(4);       //===读取编码器的值
												//===更新姿态	
		//Distance = Read_Distane();            //===获取超声波测量距离值
...
{
	u8 temp;
	if(angle<-40||angle>40||1==Flag_Stop||voltage<410)//电池电压低于11.1V关闭电机
	{	                                                 //===倾角大于40度关闭电机
...
{ 
	...
	{
		//I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_YOUT_H,&datah);//读取Y轴陀螺仪
		//I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_YOUT_L,&datal);//读取Y轴陀螺仪

		Gyro_Y = (databuf[10] << 8) + databuf[11];
		//I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_ZOUT_H,&datah);//读取Z轴陀螺仪
		//I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_ZOUT_L,&datal);//读取Z轴陀螺仪 
		Gyro_Z = (databuf[12] << 8) + databuf[13];

		//I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_XOUT_H,&datah);//读取X轴加速度计
		//I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_XOUT_L,&datal);//读取X轴加速度计		
		Accel_X = (databuf[0] << 8) + databuf[1];
		//I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_ZOUT_H,&datah);//读取Z轴加速度计
		//I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_ZOUT_L,&datal);//读取Z轴加速度计		

...
void MiniBalance_Motor_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = AIN1_PIN | AIN2_PIN | GPIO_Pin_12;	//端口配置
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;      //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;     //50M
  GPIO_Init(AIN1_PORT, &GPIO_InitStructure);					      //根据设定参数初始化GPIOB 
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE); //使能PB端口时钟
  GPIO_InitStructure.GPIO_Pin = BIN1_PIN | BIN2_PIN;	//端口配置
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;      //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;     //50M
  GPIO_Init(BIN1_PORT, &GPIO_InitStructure);					      //根据设定参数初始化GPIOB 

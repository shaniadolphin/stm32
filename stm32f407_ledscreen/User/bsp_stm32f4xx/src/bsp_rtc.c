...
//返回值:SUCEE(1),成功
//       ERROR(0),进入初始化模式失败 
unsigned char RTC_Set_Time(unsigned char hour,unsigned char min,unsigned char sec,unsigned char ampm)
{
...
//返回值:SUCEE(1),成功
//       ERROR(0),进入初始化模式失败 
unsigned char RTC_Set_Date(unsigned char year,unsigned char month,unsigned char date,unsigned char week)
{
...
//       1,LSE开启失败;
//       2,进入初始化模式失败;
unsigned char My_RTC_Init(void)
{
	...
	if(RTC_ReadBackupRegister(RTC_BKP_DR0)!=0x5050)		//是否第一次配置?
	{
		...
		RTC_Set_Time(19,49,56,RTC_H12_AM);	//设置时间
		RTC_Set_Date(18,3,19,1);		//设置日期

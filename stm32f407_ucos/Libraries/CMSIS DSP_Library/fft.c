...
void cr4_fft(int dir, long m, float *x, float *y)
{
...
void powerMag(unsigned int nfill, float *StrPara_REL, float *StrPara_IMG,float *StrPara_MAG)
{
	int X1,Y1;
	unsigned int  i;
	float X,Y,Mag;
...
void test_fft(unsigned short *dataraw, unsigned int datarawlen)
{
	unsigned int i;	
	float *datahamming={0};
	float *datareal;
...
#endif	
	//MakeInput(fFs);
	for(i = 0; i<datarawlen; i++)
	{
		datahamming[i] = 1.0f;//(1 - 0.46164) - 0.46164 * cos(2 * PI * i/(datarawlen - 1));	
		datareal[i] = datahamming[i] * (float)(dataraw[i]);
		dataimg[i] = 0;
	}	
	cr4_fft(1, 10, datareal, dataimg);
	powerMag(1024, datareal, dataimg, datamag);
	for(i=0;i<100;i++)
	{
		printk("[%d]=%d ",i,(unsigned int)(datamag[i]));	
		if(i%4==3){printk("\r\n");}	
	}
	//cr4_fft(-1,8,FFT_BUF_REL,FFT_BUF_IMG);
	//for(i=0;i<SAMPLE_NODES;i++)
	//{
	//	printf("[%d]=%x ",i,(unsigned int)(FFT_BUF_REL[i]));	
	//	if(i%4==3){printf("\r\n");}	
